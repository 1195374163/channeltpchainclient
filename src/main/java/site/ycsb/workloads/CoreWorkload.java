...
public class CoreWorkload extends Workload {
  ...
  //生成了一个基于命令行参数payload的常量的整型数，这是k-v存储的value字段中field的长度
  protected static NumberGenerator getFieldLengthGenerator(Properties p) throws WorkloadException {
    ...
    //fieldlengthdistribution默认是 constant 连续分布
  ...
  public void init(Properties p) throws WorkloadException {
    ...
    }//fieldnames 里实际有只有 field0
    
    //fieldlengthgenerator是一个基于常量的由payload指定的整型数
    ...
    //确定insertcount是100万，由recordcount-insertstart计算
    long insertcount=
  ...
  //线程实际调用的是这个，在这里选择出本次指令是什么操作，然后分发到具体的方法中
  /**
  ...
  public void doTransactionInsert(DB db) {
    ...
    try {
      ...
      // 上面的nextValue()被调用多次， 之后收到服务端的响应才会进行对值ack
  ...
  //根据一个key长整形数生成一个String类的key：生成固定长度的key：user0001
